...
assert(env.absoluteAmazonUrl("https://www.amazon.de:443/x") == "https://www.amazon.de:443/x")
assert(env.absoluteAmazonUrl("https://www.amazon.de?ref=1") == "https://www.amazon.de?ref=1")
assert(env.absoluteAmazonUrl("https://www.amazon.de#frag") == "https://www.amazon.de#frag")
local foreignOk, foreignErr = pcall(env.absoluteAmazonUrl, "https://evil.example/x")
assert(foreignOk == false, "foreign host must error")
assert(tostring(foreignErr):find("not allowed", 1, true), "foreign host error message")
local httpOk, httpErr = pcall(env.absoluteAmazonUrl, "http://www.amazon.de/x")
assert(httpOk == false, "http scheme must error")
assert(tostring(httpErr):find("only https", 1, true), "http scheme error message")
assert(env.SupportsBank(env.ProtocolWebBanking, "Amazon Bestellungen") == true, "SupportsBank Amazon Bestellungen")
assert(env.SupportsBank(env.ProtocolWebBanking, "Amazon") == false, "SupportsBank rejects legacy Amazon")
assert(env.SupportsBank(env.ProtocolWebBanking, "Amazon Orders") == false, "SupportsBank Amazon Orders")
assert(env.SupportsBank(env.ProtocolWebBanking, "Amazonian") == false, "SupportsBank prefix reject")
assert(env.SupportsBank(env.ProtocolWebBanking, "Amazon-Kreditkarte") == false, "SupportsBank Amazon-Kreditkarte")
